class ApplicationController < ActionController::Base
  ...
  def load_poll_and_ballot
    @poll = Poll.first(:conditions => {:invitation_key => params[:ballot][:poll_id]})
    @poll ||= Poll.find(params[:ballot][:poll_id] || params[:id])
    if @poll.present? && params[:ballot][:ballot_key].blank? #&& @poll.invitation_key.present?
